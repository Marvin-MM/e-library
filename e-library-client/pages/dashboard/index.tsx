            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mt-4 justify-center">
                ...
                {["Machine Learning", "Climate Change", "African History", "Renewable Energy", "Public Health"].map((tag) => (
                  <Badge
                    key={tag}
                    className="cursor-pointer hover:bg-black/50 border-white/20 backdrop-blur-sm transition-colors"
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent>
              {loadingLatest ? (
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {latestResources.slice(0, 4).map((resource) => (
                    <Link
                      key={resource.id}
                      href={`/resources/${resource.id}`}
                      className="flex items-center gap-3 p-4 rounded-xl bg-accent/50 hover:bg-accent transition-all group"
                    >
                  ...
                  {myRequests.slice(0, 4).map((request) => (
                    ...
                    >
                      <div className={`p-2 rounded-xl ${
                        request.status === "RESOLVED" ? "bg-green-500/10" :
